...
int main() {
    ...
    LazySegTree<MAsumadd<long long>> lseg(A);
    for (int i = 0; i < M; i++) {
        long long C = lseg[B[i]];
        lseg.set(B[i], 0);
        if (C >= N) {
            int a = C / N;
            lseg.apply(0, N, a);
        ...
        if (right == 0) {
            ...
            if (C <= right) {
                lseg.apply(B[i] + 1, B[i] + 1 + C, 1);
            } else {
                lseg.apply(B[i] + 1, N, 1);
                lseg.apply(0, C - right, 1);
            }
        }
    }
    vector<long long> ans = lseg.getall();
    for (int i = 0; i < N; i++) {
        if (i) cout << " ";
        cout << ans[i];
    }
